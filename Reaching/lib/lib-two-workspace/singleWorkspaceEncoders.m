function results = singleWorkspaceEncoders(td,params)
...
%             markername = 'Marker_1';
            markername = 'hand2';
            [point_exists,marker_hand_idx] = ismember(strcat(markername,'_',{'x','y','z'}),td.dlc_pos_names);
        ...
        case 'handelbow'
            ...
            markername = 'elbow1';
            [point_exists,marker_elbow_idx] = ismember(strcat(markername,'_',{'x','y','z'}),td.dlc_pos_names);
...
%% Get comparison of actual tuning curves with various modeled tuning curves
    ...
    if get_tuning_curves
        ...
        for foldctr = 1:num_folds
            ...
            for i_name = 1:size(glm_params{modelnum}.in_signals,2)
                for i_idx = 1:numel(glm_params{modelnum}.in_signals{i_name,2})
                    ...
                    td_test{1,foldctr}.(glm_params{modelnum}.in_signals{i_name,1}) = td.(glm_params{modelnum}.in_signals{i_name,1})(test_idx,:);
                    td_test{1,foldctr}.vel = td.vel(test_idx,:);
                    
                    % meta info
                    td_test{1,foldctr}.monkey = td.monkey;
                    
                    td_test{1,foldctr}.task = td.task;
                    
                    td_test{1,foldctr}.date = td.date;
                end
            end
            % get out signals
            td_train.(glm_params{modelnum}.out_signals) = ...
                td.(glm_params{modelnum}.out_signals)(train_idx,:);
            td_test{1,foldctr}.(glm_params{modelnum}.out_signals) = td.(glm_params{modelnum}.out_signals)(test_idx,:); 
            
            % set bin_size as well
            td_train.bin_size = td.bin_size;
            td_test{1,foldctr}.bin_size = td.bin_size;
            ...
            for modelnum = 1:num_models-1
                ...
                td_test{1,foldctr} = getModel(td_test{1,foldctr},glm_info);
            end
        end
        
        td_test = horzcat(td_test{1,:});
        ...
        for modelnum = 1:num_models
            ...
            pdTables{1,modelnum} = getTDClassicalPDs(td_test,pd_params);
            ...
            tuning_params = struct(...
                ...
                'meta',struct('spaceNum',0));
            tuning_curves{1,modelnum} = getTuningCurves(td_test,tuning_params);
        end
    end
...
%% Cross-validate models of neural data
    ...
    [crossEval,crossTuning,crossval_lookup] = analyzeSingleWorkspace(td,crossval_params);
