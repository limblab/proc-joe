folderpath = 'C:\Users\Joseph\Desktop\Lab\Data\StimArtifact\ArtificialMonkey_20180227_stimSwitch\';
...
pwd=cd;
cd(folderpath)
fileList = dir('*interpulse300*nev*');
...
%         
ts = 50;
artifact = zeros(2,numel(stimOn),30*ts); % 5ms of data
for i = 1:numel(stimOn)
%     artifact(1,i,:) = cds.analog{1,1}.ainp15(stimOn(i):stimOn(i)+30*ts-1);
    artifact(2,i,:) = cds.lfp{stimOn(i):stimOn(i)+30*ts-1,3};
end
...
for i = 1:size(artifact,2)
    subplot(6,6,i)
    plot(xData,squeeze(artifact(1:10:100,i,:)));
    ylim([-10000,10000])
%     ylim([-500,500])
    xlim([0,5])
    title(num2str(i));
