...
stimIdx = 5;
artifactAll = squeeze(artifactData.artifact(:,stimIdx,:));

% this is where we would want to place spikes
spikeIdxAll = zeros(size(artifactAll,1),1)-1;
waveIdxAll = zeros(size(artifactAll,1),1)-1;
if(placeSpikes)
...
%         waveIdx = ceil(rand()*size(neuronMeanWave,1));
        waveIdx = 1;
        artifactAll(ch,spikeIdx:spikeIdx+size(neuronMeanWave,2)-1) = ... 
            artifactAll(ch,spikeIdx:spikeIdx+size(neuronMeanWave,2)-1) + neuronMeanWave(waveIdx,:)/8;
...
stimIdx = 5;
artifactAll = squeeze(artifactData.artifact(:,stimIdx,100:end));
...
%         spikeIdx = ceil(rand()*(size(artifactAll,2)-size(neuronMeanWave,2)-2));
        ...
        waveIdx = ceil(rand()*size(neuronMeanWave,1));
        artifactAll(ch,spikeIdx:spikeIdx+size(neuronMeanWave,2)-1) = ... 
            artifactAll(ch,spikeIdx:spikeIdx+size(neuronMeanWave,2)-1) + neuronMeanWave(waveIdx,:);
...
for ch = 1:96
    disp(ch)
    data = artifactAll(ch,startIdx:endIdx);
    y=data';
    x = ((startIdx:1:endIdx)');
    expEqn = 'poly4';
    N = 50;
    ...
    data = y;
    artifactAll(ch,startIdx:endIdx) = data';
end
% plot each channel
eList = outputData.eList;
posList = outputData.posList;
...
for elec = 1:96
    ...
    h=subplot(10,10,10*(eRow-1)+eCol);
    hold on
    f=plot(1:1:numel(artifactAll(elec,:)),artifactAll(elec,:),'linewidth',2,'color','k');
    if(spikeIdxAll(elec) ~= -1)
        plot(spikeIdxAll(elec):spikeIdxAll(elec)+size(neuronMeanWave,2)-1,artifactAll(elec,spikeIdxAll(elec):spikeIdxAll(elec)+size(neuronMeanWave,2)-1),'linewidth',2,'color','r')
    end
    ylim([-1000,1000])
end
% 
...
%% ICA on one channel across all stimulations
artifactOneChan = squeeze(artifactData.artifact(73,:,:));
artifactOneStim = squeeze(artifactData.artifact(:,73,:));
[source,A,W] = fastica(artifactOneChan,'g','pow3','finetune','tanh','approach','defl','epsilon',0.00001);
figure
subplot(2,1,1)
