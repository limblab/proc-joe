...
for i = 2:numel(t)-1
    % estimators
    theta0(i,1) = sum(data(1:i,1))/(t(i));
    theta1(i,1) = sum(data(i+1:end,1))/(t(end)-t(i));
    tau(i,1) = t(i) + (data(i+1,1) - theta1(i,1))/(theta0(i,1)-theta1(i,1));
    if(tau(i,1) < t(i)-binSize/2)
        tau(i,1) = t(i)-binSize/2;
    elseif(tau(i,1) > t(i)+binSize/2)
        tau(i,1) = t(i)+binSize/2;
    end
    % likelihood
