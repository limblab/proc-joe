%% set initial parameters
    ...
    input_data.folderpath = 'C:\Users\Joseph\Desktop\Lab\Data\Han_stim_evoked_move\Han_20200825_manyChans\';
...
%% make cds
    ...
    params.event_list = {'goCueTime';'bumpTime';'bumpMagnitude';'stimTime';'stimCode'};
...
%% plot trajectory from before cue (stim or bump) to end of trial?
    ...
    for i_trial = 1:numel(td_plot)
         ...
         if(plot_mode == 1)
             ...
             idx_plot = td_plot(i_trial).idx_stimTime + ceil(0.12/td_plot(i_trial).bin_size);
         ...
         elseif(plot_mode == 2)
             ...
             idx_plot = td_plot(i_trial).idx_bumpTime + ceil(0.12/td_plot(i_trial).bin_size);
             plot(td_plot(i_trial).pos(idx_plot,1),td_plot(i_trial).pos(idx_plot,2),'b.','markersize',12)
