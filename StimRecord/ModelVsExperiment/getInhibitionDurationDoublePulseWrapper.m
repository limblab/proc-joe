function [output_data] = getInhibitionDurationDoubplePulseWrapper(array_data,input_data)
    ...
    for i_unit = 1:numel(array_data)
        ...
        for i_cond = 1:numel(input_data.cond_list)
            ...
            if(numel(array_data{i_unit}.num_stims) == 8) % 
                
                % update post window and blank time to look at last pulse
                % in sequence
                input_data.post_window = input_data.post_window + array_data{i_unit}.PULSE_TIMES{1}{1}(end);
                input_data.blank_time = input_data.blank_time + array_data{i_unit}.PULSE_TIMES{1}{1}(end);
                
                temp_inhib_data = getInhibitionDuration(array_data{i_unit},input_data.cond_list(i_cond),input_data);
