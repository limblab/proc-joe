...
if ~noTrials
    ...
    % if there's no trial table (or it's empty), populate
    mult_fact = 100;
    if(isempty(trials))
        trials = table(ceil(cds.meta.dataWindow(1)*mult_fact)/mult_fact,ceil(cds.meta.dataWindow(1)*mult_fact)/mult_fact,...
            floor(cds.meta.dataWindow(end)*mult_fact)/mult_fact,floor(cds.meta.dataWindow(end)*mult_fact)/mult_fact,{'R'},'VariableNames',{'start_time','startTime','endTime','end_time','result'});
    else % else overwrite first entry.
        trials.start_time(1) = ceil(cds.meta.dataWindow(1)*mult_fact)/mult_fact;
        trials.startTime(1) = ceil(cds.meta.dataWindow(1)*mult_fact)/mult_fact;
        trials.endTime(1) = floor(cds.meta.dataWindow(end)*mult_fact)/mult_fact;
        trials.end_time(1) = floor(cds.meta.dataWindow(end)*mult_fact)/mult_fact;
        trials.result(1) = 'R';
    end
...
elseif(~isempty(cds.cursor))
    bin_size = round(1000*mode(diff(cds.cursor.t)))/1000;
else
    bin_size = .01;
end
...
elseif(~isempty(cds.cursor))
    cds_bin.kin = decimate_signals(cds.cursor,kin_list,bin_size);
end
...
for i=1:length(cds.analog)
    header = cds.analog{i}.Properties.VariableNames;
    if any(contains(header,'ncams'))
        dlc_analog_idx = i;
        break
    end
end
if dlc_analog_idx > 0
    dlc_data=cds.analog{dlc_analog_idx};
    dlc_list = cds.analog{dlc_analog_idx}.Properties.VariableNames;
    % split dlc_data into dlc_pos (marker data) and dlc_score (ncams and
    % error)
    % pos data has _x, _y, _z at end
    is_pos_mask = zeros(1,size(dlc_data,2));
    for i_name = 1:numel(dlc_list)
        if(~isempty(strfind(dlc_list{i_name},'_x')) || ...
                ~isempty(strfind(dlc_list{i_name},'_y')) || ...
                ~isempty(strfind(dlc_list{i_name},'_z')))
            is_pos_mask(i_name) = 1;
            % convert dlc data from mm to cm
            dlc_data.(dlc_list{i_name}) = dlc_data.(dlc_list{i_name})/10;
        end
    end
    
    dlc_pos_list = dlc_list(is_pos_mask==1);
    dlc_score_list = dlc_list(is_pos_mask==0);
    
    dlc_pos = dlc_data(:,is_pos_mask==1);
    dlc_score = dlc_data(:,is_pos_mask==0);
...
for i = 1:length(idx_trials)
    ...
    if ~isempty(idx)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Add DLC data
        if isfield(cds_bin,'dlc_pos') && ~isempty(cds_bin.dlc_pos)
            % add marker data
            fn = dlc_pos_list;
            fn = fn(~strcmpi(fn,'t'));
            
            trial_data(i).dlc_pos = zeros(length(idx),length(fn));
            
            % loop along the markers to decimate
            for entry = 1:length(fn)
                temp = cds_bin.dlc_pos.(fn{entry});
                trial_data(i).dlc_pos(:,entry) = temp(idx);
            end
            % add dlc marker names
            trial_data(i).dlc_pos_names = fn;
            
            % add score data
            fn = dlc_score_list;
            fn = fn(~strcmpi(fn,'t'));
            
            trial_data(i).dlc_score = zeros(length(idx),length(fn));
            
            % loop along the markers to decimate
            for entry = 1:length(fn)
                temp = cds_bin.dlc_score.(fn{entry});
                trial_data(i).dlc_score(:,entry) = temp(idx);
            end
            % add dlc marker names
            trial_data(i).dlc_score_names = fn;
        end
