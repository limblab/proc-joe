...
%                     monitorCh2Mat(j-1,k,1:inputData.windowSize+inputData.presample)=reshape(cds.ananlog{mIdx}.([monitorName,'2'])(stimWindows(k,1)-inputData.presample:stimWindows(k,2)),[1,1,inputData.windowSize+inputData.presample]);
    ...
    for artIdx = 1:numel(outputData.artifactData)
        outputData.artifactData(artIdx).artifactFiltered = outputData.artifactData(artIdx).artifact;
        for chanIdx = 1:size(outputData.artifactData(artIdx).artifact,1)
            outputData.artifactData(artIdx).artifactFiltered(chanIdx,:,:) = acausalFilter(squeeze(outputData.artifactData(artIdx).artifact(chanIdx,:,:))')';
        end
    end
