...
%                 error('processStimArtifact:cantFindSync','couldnt find a sync signal')
        ...
        artifact = zeros(numel(artifactDataPre.stimOn)-1,33,1000);
        for st = 1:numel(artifactDataPre.stimOn)-1
            artifact(st,1:32,:) = squeeze(cdsTempLFP(artifactDataPre.stimOn(st)-99:artifactDataPre.stimOn(st)+900,2:33)');
            artifact(st,33,:) = squeeze(cdsTemp.analog{1,1}.ainp15(artifactDataPre.stimOn(st)-99:artifactDataPre.stimOn(st)+900));
        end
    end
