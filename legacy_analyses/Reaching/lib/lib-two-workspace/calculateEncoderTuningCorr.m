function tuning_corr = calculateEncoderTuningCorr(encoderResults,params)
    ...
    for tablerownum = 1:height(crossTuningPM)
        real_tuning_shape = horzcat(...
            crossTuningPM(tablerownum,:).(sprintf('%s_dlc_vel_handCurve',neural_signal)),...
            crossTuningDL(tablerownum,:).(sprintf('%s_dlc_vel_handCurve',neural_signal)))';
        ...
        for modelnum = 1:length(model_aliases)
            ...
            model_tuning_shape(:,modelnum) = horzcat(...
                crossTuningPM(tablerownum,:).(sprintf('glm_%s_model_dlc_vel_handCurve',model_aliases{modelnum})),...
                crossTuningDL(tablerownum,:).(sprintf('glm_%s_model_dlc_vel_handCurve',model_aliases{modelnum})))';
        end
