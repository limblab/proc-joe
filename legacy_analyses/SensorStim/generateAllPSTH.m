    ...
    if(cds.units(i).ID ~= 0 && cds.units(i).ID ~=255) % unsorted stuff
        if(useZeroAsStart)
            [binEdges,binCounts]=generatePESTH(cds, i, 'zeroEvent', 'start','sequenceTimes',sequenceTimes,'eventTimes',eventTimes,...
                'stimState',stimState,'noPlots',noPlots,'useRate',useRate, 'return', ret,'binSize',binSize,'spindleStim',spindleStim);
        else
        ...
        if(~spindleStim && ret && keepNeuronGTOstim(cds, i, binEdges, binCounts, sequenceTimes,eventTimes,useRate)==1) % determine if keeping
            neuronRet = [neuronRet; i];
        elseif(spindleStim && ret && keepNeuronSpindleStim(cds, i, binEdges, binCounts, sequenceTimes, eventTimes,useRate,stimState)==1) % determine if keeping
            neuronRet = [neuronRet; i];
        end
    end 
end
