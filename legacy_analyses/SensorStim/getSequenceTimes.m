    ...
    if(stimState(i)~=stimState(i-1)) % change in stim state
        ...
        if(stimState(i) == 1) 
            if(GTOstim)
                ...
                sequenceTimes = [sequenceTimes; cds.analog{1,1}.t(i)-cds.analog{1,1}.t(1), -1];
                if(~useEndAsZero)
                    eventTimes = [eventTimes; cds.analog{1,1}.t(i)-cds.analog{1,1}.t(1)];
                end
            end
        else % change is off
            if(GTOstim)
                ...
                sequenceTimes(end,2) = cds.analog{1,1}.t(i)-cds.analog{1,1}.t(1);
                if(useEndAsZero)
                    eventTimes = [eventTimes; cds.analog{1,1}.t(i)-cds.analog{1,1}.t(1)];
                end
            end
